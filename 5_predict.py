...
if os.path.exists(cor_t1_c):
    ...
    model.load_weights(os.path.join(model_folder, 'segmentation.weights.h5'))
    ...
    slice_w = os.path.join(model_folder, 'slice-selection.weights.h5')
    use_slice_selection = os.path.exists(slice_w)
    if use_slice_selection:
        model.load_weights(slice_w)
    else:
        print(">> [predict] slice-selection weights not found, skip slice selection and segment full volume.")
